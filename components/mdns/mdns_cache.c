                                                            ...
                                                            uint32_t ttl)
{
    ...
    for (mdns_cache_subtype_t *it = service_entry->subtype_list; it; it = it->next) {
        if (names_equal(it->subtype, subtype)) {
            it->expires_at_us = calc_expiration_time_us(ttl);
    ...
    subtype_entry->expires_at_us = calc_expiration_time_us(ttl);
                                                      ...
                                                      const char *subtype, uint32_t ttl)
{
    ...
    if (ttl == 0) {
        ...
        if (!mdns_priv_browse_notify_ptr_goodbye_from_service_cache(owner_entry, service_entry, subtype)) {
        ...
        if (base_ptr_updated) {
            ...
            service_cache_remove_subtype(service_entry, subtype);
        }
    ...
    if (!base_ptr_updated) {
        ...
        service_entry->ptr_expires_at_us = calc_expiration_time_us(ttl);
                                                           ...
                                                           uint16_t port, uint32_t ttl)
{
    ...
    cache->srv_expires_at_us = calc_expiration_time_us(ttl);
                                                           ...
                                                           mdns_txt_linked_item_t *new_txt, uint32_t ttl)
{
    ...
    service_entry->txt_expires_at_us = calc_expiration_time_us(ttl);
                                                    ...
                                                    bool create_if_absent)
{
    ...
    if (!addr_entry) {
        ...
        new_addr->addr = *addr;
        new_addr->ttl = ttl;
        new_addr->expires_at_us = calc_expiration_time_us(ttl);
        ...
        while (*service_ptr) {
            ...
            if (addr_removed) {
                service_cache_mark_sync_out(service, MDNS_CACHE_UPDATED,
                                            MDNS_CACHE_RECORD_ADDR, MDNS_CACHE_CONSUMER_BROWSE);
            }
            ...
            if (service->srv_present && now_us >= service->srv_expires_at_us) {
                ...
                service->port = 0;
                service->srv_ttl = 0;
                service->srv_expires_at_us = 0;
                service_cache_mark_sync_out(service, MDNS_CACHE_UPDATED,
                                            MDNS_CACHE_RECORD_SRV, MDNS_CACHE_CONSUMER_BROWSE);
            }
            ...
            if (service->txt_present && now_us >= service->txt_expires_at_us) {
                ...
                service->txt_ttl = 0;
                service->txt_expires_at_us = 0;
                service_cache_mark_sync_out(service, MDNS_CACHE_UPDATED,
                                            MDNS_CACHE_RECORD_TXT, MDNS_CACHE_CONSUMER_BROWSE);
            }

            if (service->ptr_present && now_us >= service->ptr_expires_at_us) {
                // Notify browses before removing PTR record
                ...
                service->ptr_ttl = 0;
                service->ptr_expires_at_us = 0;
            }

            mdns_cache_subtype_t **subtype_ptr = &service->subtype_list;
            while (*subtype_ptr) {
                mdns_cache_subtype_t *subtype = *subtype_ptr;
                if (now_us >= subtype->expires_at_us) {
