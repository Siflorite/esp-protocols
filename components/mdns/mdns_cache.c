...
static void service_cache_mark_dirty(mdns_service_cache_t *service_entry, mdns_cache_update_result_t result,
                                     mdns_cache_record_type_t record_type, mdns_cache_consumer_type_t consumer)
{
    if (service_entry && (result == MDNS_CACHE_ADDED || result == MDNS_CACHE_UPDATED)) {
        service_entry->dirty_records |= (uint8_t)record_type;
        service_entry->dirty_consumers |= (uint8_t)consumer;
    }
}

/**
 * @brief Clear dirty flags for a service cache.
 *
 * @note For browsers, only clear MDNS_CACHE_DIRTY_BROWSE from dirty_consumers.
 *
 * @param service_entry Service cache entry to clear dirty flags for.
 * @param consumer Consumer type to clear dirty flags for.
 * @param record_type Record type to clear dirty flags for.
 */
static void service_cache_clear_dirty(mdns_service_cache_t *service_entry, mdns_cache_consumer_type_t consumer,
                                      mdns_cache_record_type_t record_type)
{
    if (service_entry) {
        switch (consumer) {
                                                      ...
                                                      const char *subtype, uint32_t ttl)
{
    ...
    mdns_service_cache_t *service_entry = mdns_priv_cache_find_service(esp_netif, ip_protocol, instance,
    ...
    if (ttl == 0) {
        ...
        service_cache_mark_dirty(service_entry, MDNS_CACHE_UPDATED,
...
{
    mdns_cache_entry_t *owner_entry = NULL;
    mdns_service_cache_t *service_entry = mdns_priv_cache_find_service(esp_netif, ip_protocol, instance, service,
                                                                       proto, &owner_entry);
    ...
    if (ttl == 0) {
        ...
        service_cache_mark_dirty(service_entry, MDNS_CACHE_UPDATED, MDNS_CACHE_RECORD_SRV, MDNS_CACHE_CONSUMER_BROWSE);
    ...
    service_cache_mark_dirty(service_entry, result, MDNS_CACHE_RECORD_SRV, MDNS_CACHE_CONSUMER_BROWSE);
    return result;
}
                                                      ...
                                                      mdns_txt_linked_item_t *txt, uint32_t ttl)
{
    ...
    if (ttl == 0) {
        ...
        service_cache_mark_dirty(service_entry, MDNS_CACHE_UPDATED, MDNS_CACHE_RECORD_TXT, MDNS_CACHE_CONSUMER_BROWSE);
    ...
    service_cache_mark_dirty(service_entry, result, MDNS_CACHE_RECORD_TXT, MDNS_CACHE_CONSUMER_BROWSE);
    return result;
}
                                                       ...
                                                       const char *hostname, const esp_ip_addr_t *addr, uint32_t ttl)
{
            ...
            if (addr_equal(&(*addr_ptr)->addr, addr)) {
                ...
                *addr_ptr = removed_addr->next;
                ...
                for (mdns_service_cache_t *service = entry->service_cache_list; service; service = service->next) {
                    service_cache_mark_dirty(service, MDNS_CACHE_UPDATED, MDNS_CACHE_RECORD_ADDR, MDNS_CACHE_CONSUMER_RESOLVER);
                }
    ...
    for (mdns_service_cache_t *service = entry->service_cache_list; service; service = service->next) {
        service_cache_mark_dirty(service, result, MDNS_CACHE_RECORD_ADDR, MDNS_CACHE_CONSUMER_BROWSE);
    }
...
void mdns_priv_cache_process_dirty(void)
{
    for (mdns_cache_entry_t *entry = s_cache; entry; entry = entry->next) {
        for (mdns_service_cache_t *service = entry->service_cache_list; service; service = service->next) {
            if (service->dirty_consumers & MDNS_CACHE_CONSUMER_BROWSE) {
                if (mdns_priv_browse_update_from_service_cache(entry, service)) {
                    service_cache_clear_dirty(service, MDNS_CACHE_CONSUMER_BROWSE, MDNS_CACHE_RECORD_PTR);
                }
            }

            // TODO: When resolver is implemented, add resolver dirty processing here.
            // if (service->dirty_consumers & MDNS_CACHE_CONSUMER_RESOLVER) {
            //     if (mdns_priv_resolver_update_from_service_cache(..., service->dirty_records)) {
            //         service_cache_clear_dirty(service, MDNS_CACHE_CONSUMER_RESOLVER, MDNS_CACHE_RECORD_ADDR);
            //     }
            // }
        }
    }
